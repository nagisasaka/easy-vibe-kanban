...
状態: **コード照合レビュー反映済み・実装前。第2.2節の範囲判断は未確定**

> 基準mainと未commit変更を含む作業ツリーを静的に照合した。SHA・既存シンボル・実行未検証の範囲は [レビューガイド](00-review-guide.md) 第2節を参照。本書の追加契約が実装済みとは扱わない。既存manual mergeのsquash／source ref更新／完了通知を、そのまま正式Integrationの反映処理へ流用しない。
...
以下は最小案であり、対応範囲を狭める選択を本改訂で合意済みにはしない。実装に着手する際に採用範囲と未対応表示を確定する。
...
これらは追加する受入条件であり、現時点で実行済みではない。通常Sessionの終了処理、既存manual merge、OpenWiki Sync、goal／queueの回帰を、変更する共通境界のテストにも含める。
